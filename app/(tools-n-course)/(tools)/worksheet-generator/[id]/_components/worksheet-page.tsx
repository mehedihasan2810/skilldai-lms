...
export const WorksheetPage = ({
  ...
  const {
    ...
    error,
    isLoading,
  } = useQuery<{ title: string; worksheets: string }>({
    queryKey: ["worksheet", id],
    queryFn: async () => await getWorksheet({ worksheetId: id }),
  });

  const handleCopy = async () => {
    try {
      const texts = markdownToTxt(worksheet?.worksheets ?? "");
      await navigator.clipboard.writeText(`${worksheet?.title}\n\n${texts}`);
      setIsCopied(true);
  ...
  const downloadTxtFile = () => {
    ...
    const texts = markdownToTxt(worksheet?.worksheets ?? "");
    const file = new Blob([`${worksheet?.title}\n\n${texts}`], {
      type: "text/plain",
    });
    element.href = URL.createObjectURL(file);
    element.download = "my-worksheet.txt";
    // document.body.appendChild(element); // Required for this to work in FireFox
    element.click();
  };

  const handlePrint = () => {
    const plainText = markdownToTxt(worksheet?.worksheets ?? "");
    // Wrap the plain text in preformatted tags for better readability
    const formattedText = `<pre>${worksheet?.title}\n\n${plainText}</pre>`;
