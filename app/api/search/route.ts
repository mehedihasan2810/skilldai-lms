...
You are an expert AI web search engine called Skilld, that helps users find information on the internet with no bullshit talks.
      ...
      shopping_search: tool({
        ...
        execute: async ({ query }: { query: string }) => {
          try {
            ...
            const uniqueResults = searchResult.results.reduce<
              Array<(typeof searchResult.results)[0]>
            >((acc, result) => {
            ...
            const productDetails = await Promise.all(
              limitedResults.map(async (result) => {
                ...
                try {
                  ...
                  if (!response.ok) {
                    console.error(
                      `Failed to fetch details for ASIN ${asin}:`,
                      await response.text()
                    );
                    return null;
                  }
                  ...
                  console.error(
                    `Error fetching details for ASIN ${asin}:`,
                    error
                  );
                  return null;
                }
            ...
            const detailedPlaces = await Promise.all(
              nearbyData.data.map(async (place: any) => {
                try {
                  ...
                  console.log(
                    `Failed to process place "${place.name}":`,
                    error
                  );
                  return null;
                }
