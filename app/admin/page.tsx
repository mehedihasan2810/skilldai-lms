...
export default function AdminPage() {
  ...
  useEffect(() => {
    const checkAdminStatus = async () => {
      try {
        const userRes = await fetch("/api/novu/getLoggedUser");
        const userData = await userRes.json();
        if (!userRes.ok) throw new Error("Failed to get user data");

        const adminRes = await fetch("/api/novu/check-admin", {
          method: "POST",
  ...
  if (isAdmin === null) {
    return <p className="text-center text-white">Checking admin status...</p>;
  }
      ...
      <Card className="max-w-xl w-full p-8 shadow-lg rounded-lg text-white border border-gray-700">
        ...
        <form onSubmit={sendNotification} className="space-y-5">
          ...
          <div className="flex items-center justify-between">
            ...
            <Checkbox
  checked={isInApp}
  onCheckedChange={(checked) => setIsInApp(checked === true)}
/>
            ...
            <Checkbox
  checked={isEmail}
  onCheckedChange={(checked) => setIsEmail(checked === true)}
/>
